...
import ToothTreatmentStep from './substeps/ToothTreatmentStep';
import SectionTreatmentStep from './substeps/SectionServiceStep';
import GeneralTreatmentStep from './substeps/GeneralServiceStep';

type CheckupStepProps = {
  checkup: DentalCheckupResponseType
}
...
const CheckupStep = ({
  ...
  return (
    <section className="flex items-center justify-start gap-6 flex-col h-full pt-6 rounded-b-3xl overflow-y-scroll ">
      <div className="relative flex items-center justify-center bg-gray-50  w-fit rounded-md p-1 h-8 flex-shrink-0 ">
        ...
        <div className="flex flex-col items-center justify-center gap-1">
          ...
          <div className="bg-green-50 rounded-md px-2 py-1 flex items-center justify-center gap-1 ">
            ...
            <label className="text-sm font-normal text-green-500 ">Click on a tooth to view the treatment.</label>
          </div>
          }          
          {activeStep === "section" && (
            <div className="bg-violet-50 rounded-md px-2 py-1 flex items-center justify-center gap-1 ">
              ...
              <label className="text-sm font-normal text-violet-500 ">Select a section to view the treatment.</label>          
            </div>
          )}      
          {activeStep === "general" && (
            <div className="bg-indigo-50 rounded-md px-2 py-1 flex items-center justify-center gap-1 ">
              <InformationIcon className="w-5 h-5 stroke-2 stroke-white fill-indigo-500" />
              <label className="text-sm font-normal text-indigo-500 ">
                Here you can view additional details about your treatments:
              </label>
            </div>
          )}    
        </div>
      {activeStep === "tooth" && <ToothTreatmentStep toothCheckups={checkup.checkupData.toothCheckup} />}
      {activeStep === "section" && <SectionTreatmentStep sectionCheckups={checkup.checkupData.sectionCheckup} />}
      {activeStep === "general" && <GeneralTreatmentStep generalCheckups={checkup.checkupData.generalCheckup} />}
    </section>
  );
}
