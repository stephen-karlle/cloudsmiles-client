...
const PrivateChatbox: FC <IChatbox> = ({ lastMessage, message, setMessage, isGenerating, handleSendMessage}) => {
  ...
  const isVisible = 
  !lastMessage 
  || lastMessage?.messageComponent === "message" 
  || lastMessage?.messageComponent === "appointmentCreated"
  || lastMessage?.messageComponent === "error"
  || lastMessage?.messageComponent === "boolean"
  
  return (
    <article className="h-auto px-4 pb-4 flex flex-col ">
      {isVisible &&(
        <section className={`flex items-center justify-center gap-4 transition-all duration-400 ease-out ${isVisible ? "h-full " : " h-0"}`}>
