...
import { generateDentistWeeklySchedule, generateWeekGridCol } from "../../../utils/calendar.utils"; 
...
const WeekSubHeader = () => {
  ...
  const { data: dentists, isLoading, isError } = useWeekDentist()

  const date = useCalendarStore((state) => state.date);
  const gridColsStyle = generateWeekGridCol(dentists, 250)
  ...
  return (
    <header className="  sticky top-0 z-40 flex flex-col">
      <article className="grid w-full bg-white " style={{gridTemplateColumns: gridColsStyle}}>
        <section className="flex flex-col items-center justify-center h-20 border-r border-b border-gray-20 sticky top-0 left-0 z-50 w-[96px] bg-white" />
